...
class DailyWeather(BaseModel):
    ...
    max_temp: List[float] = Field(..., min_items=7, max_items=7, description="Temperaturas máximas (°C)")
    min_temp: List[float] = Field(..., min_items=7, max_items=7, description="Temperaturas mínimas (°C)")
    precipitation: List[float] = Field(..., min_items=7, max_items=7, description="Precipitación (mm)")
    dates: List[str] = Field(..., min_items=7, max_items=7, description="Fechas en formato ISO")
...
class ClimateResponse(BaseModel):
    ...
    timestamp: str = Field(..., description="Marca de tiempo de la actualización")
